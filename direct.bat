...
SET BUCKET_TAGS=--stagingLocation=%DATAFLOW_BUCKET%/staging/%VERSION% --gcpTempLocation=%DATAFLOW_BUCKET%/temp/%VERSION%


rem mvn clean package %with_extra%
mvn %with_extra% exec:java -Dexec.mainClass=com.github.cunvoas.Bq2DsAll -Dexec.args="%PROJECT_TAGS% %REGION_TAGS% %WORKER%  %BUCKET_TAGS% --runner=DirectRunner"
